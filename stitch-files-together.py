...
df = pd.read_csv(filelist[0], header=0, usecols=[i for i in range(1,11)], index_col=False)
for ii in range(1,len(filelist)):
    df_temp = pd.read_csv(filelist[ii], header=0, usecols=[i for i in range(1,11)], index_col=False)
